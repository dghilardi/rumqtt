...
pub struct EventLoop {
    ...
    reconnection_strategy: Option<Box<dyn ReconnectionStrategy + Send>>,
}
...
impl EventLoop {
    ...
    pub fn set_reconnection_strategy(&mut self, reconnection_strategy: Box<dyn ReconnectionStrategy + Send>) {
    ...
    pub async fn poll(&mut self) -> Result<Event, ConnectionError> {
        if self.network.is_none() {
            let next_attempt_opt = self.next_attempt_ts()?;
    ...
    fn next_attempt_ts(&self) -> Result<Option<Instant>, ConnectionError> {
